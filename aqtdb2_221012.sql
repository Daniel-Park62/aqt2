...
DECLARE cur1 CURSOR FOR
	SELECT uri FROM tloaddata GROUP BY uri ;
	
DECLARE cur2 CURSOR FOR 
	SELECT pkey, o_stime, rtime, elapsed, srcip, srcport, dstip, dstport, proto, method, 
			seqno, ackno, slen, rlen, sdata, rdata, errinfo
	 FROM tloaddata t
	WHERE uri = s_uri
	  AND NOT EXISTS (SELECT 1 FROM ttcppacket WHERE tcode = p_tcode AND cmpid = t.pkey)
...
readuri: LOOP
	...
	if ( (SELECT COUNT(1) FROM ttcppacket WHERE tcode = p_tcode AND uri = s_uri) >= p_ecnt ) then ITERATE readuri; END if ;
	...
	BEGIN 
		...
		insloop: LOOP
			...
			INSERT INTO ttcppacket ( tcode, cmpid, o_stime, stime, rtime, elapsed, srcip, srcport, dstip, dstport, proto, method, 
											 uri, seqno, ackno, slen, rlen, sdata, rdata, errinfo		)
								VALUES (p_tcode, vrec.pkey, vrec.o_stime, vrec.o_stime, vrec.rtime, vrec.elapsed, vrec.srcip, vrec.srcport, 
									vrec.dstip, vrec.dstport, vrec.proto, vrec.method, 
									s_uri, vrec.seqno, vrec.ackno, vrec.slen, vrec.rlen, vrec.sdata, vrec.rdata, vrec.errinfo		) ;
		END loop ;
	END;
	close cur2;
	SET done := FALSE ;
...
SET @SQLT = CONCAT ( 
	'create or REPLACE VIEW VTLOADDATA AS
	  SELECT t.* FROM TLOADDATA t  WHERE TCODE = \'',p_src,'\'', p_cond ) ; 
...
readuri: LOOP
	FETCH cur1 INTO s_uri ;
	if done then leave readuri ; 	END if ;
	if ( (SELECT COUNT(1) FROM ttcppacket WHERE tcode = p_dst AND uri = s_uri) >= p_ecnt ) then ITERATE readuri; END if ;
	...
	BEGIN 
		...
		insloop: LOOP
			...
			INSERT INTO ttcppacket ( tcode, cmpid, o_stime, stime, rtime, elapsed, srcip, srcport, dstip, dstport, proto, method, 
											 uri, seqno, ackno, slen, rlen, sdata, rdata, errinfo		)
								VALUES (p_dst, vrec.pkey, vrec.o_stime, vrec.o_stime, vrec.rtime, vrec.elapsed, vrec.srcip, vrec.srcport, 
									vrec.dstip, vrec.dstport, vrec.proto, vrec.method, 
									s_uri, vrec.seqno, vrec.ackno, vrec.slen, vrec.rlen, vrec.sdata, vrec.rdata, vrec.errinfo		) ;
			SET icnt = icnt + 1;
		END loop ;
...
SELECT CONCAT( icnt, ' 건 복제되었음') INTO v_msg ;
/*	
...
CREATE TABLE IF NOT EXISTS `tloaddata` (
  ...
  `svctime` double(22,3) GENERATED ALWAYS AS (`rtime` - `stime`) VIRTUAL,
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vtloaddata` AS SELECT t.* FROM TLOADDATA t  WHERE TCODE = 'k003' and ( stime like '2022-09-27 13:34%' ) ;
